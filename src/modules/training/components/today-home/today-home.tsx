...
import { ArrowLeft, CheckCircle2, ChevronDown, Play } from 'lucide-react'
...
export function TodayHome({ plans }: { plans: PlanTree[] }) {
  ...
  const [checked, setChecked] = useState<{ workoutId: number; state: SessionState } | null>(null)
  ...
  useEffect(() => {
    ...
    sdk
      ...
      .then((result) => {
        ...
        setChecked({ workoutId, state })
      })
      .catch(() => {
        if (active) setChecked({ workoutId, state: 'fresh' })
      })
    return () => {
      active = false
    }
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="rounded-2xl border border-ui-border-base bg-ui-bg-component p-5">
        <div className="mb-1 flex items-center gap-2 text-xs text-ui-fg-muted">
        ...
        <h2 className="text-2xl font-bold leading-tight text-ui-fg-base">{activeWorkout.title}</h2>
        ...
        {sessionState === 'completed' ? (
          <div className="mt-4 flex items-center gap-2 rounded-xl bg-ui-bg-base px-3 py-2.5 text-sm">
